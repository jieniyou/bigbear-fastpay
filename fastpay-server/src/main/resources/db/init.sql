...
CREATE TABLE `fp_admin` (
    ...
    `last_login_ip` VARCHAR(64) DEFAULT NULL COMMENT '最后登录IP地址（仅保存单个合法IPv4/IPv6）',
...
CREATE TABLE `fp_merchant` (
    ...
    `notify_url` VARCHAR(2048) DEFAULT NULL COMMENT '支付成功回调地址（用户支付成功后，平台会将支付结果回推给商户系统）',
    `return_url` VARCHAR(2048) DEFAULT NULL COMMENT '支付成功跳转地址（默认商户支付成功跳转地址，创建订单时传了returnUrl会以传入的为准）',
...
CREATE TABLE `fp_pay_order` (
    ...
    `notify_url` VARCHAR(2048) DEFAULT NULL COMMENT '回调通知地址（支付成功后通知商户系统的URL）',
    `return_url` VARCHAR(2048) DEFAULT NULL COMMENT '支付成功跳转地址（支付成功后用户浏览器跳转的URL）',
